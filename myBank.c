...
void open(float real){
	...
	if(real < 0){
		...
		if(counter == 50 || accounts[49][STATUS] == OPEN){
			...
			for(int i = 0; i < 50; i++){
				...
				if(accounts[i][STATUS] == CLOSED){
					...
					accounts[i][DEPOSIT] = real;

					accounts[i][ACCOUNTNUMBER] = i + 901;

					num = accounts[i][ACCOUNTNUMBER];
...
void balance(int whole){
	...
	for(int i = 0; i < 50; i++){
		...
		if(whole == accounts[i][ACCOUNTNUMBER] && accounts[i][STATUS] == OPEN){

			current = accounts[i][DEPOSIT];
...

	{
		...
		for(int i = 0; i < 50; i++){
			...
			if(whole == accounts[i][ACCOUNTNUMBER] && accounts[i][STATUS] == OPEN){
				...
				accounts[i][DEPOSIT] = accounts[i][DEPOSIT] + real;
	...
	if(real < 0){
		...
		for(int i = 0;i < 50; i++){
			...
			if(accounts[i][STATUS] == OPEN){
				...
				accounts[i][DEPOSIT] = accounts[i][DEPOSIT] + (accounts[i][DEPOSIT] * (real / 100));
